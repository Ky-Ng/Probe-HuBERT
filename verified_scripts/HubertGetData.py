...
Segs = ['iy', 'ih', 'ey', 'eh', 'ae', 'uw', 'uh', 'ow', 'ao', 'aa', 'ax', 'ah', 'er',
        ...
        'th', 'v', 'dh', 'b', 'd', 'g', 'p', 't', 'k']
N = 200
# lst = sorted(glob.glob('../TIMIT-Database/TIMIT/TEST/DR3/*/*.wav'))
# lst = sorted(glob.glob('/Users/kyleng/B_Organized/A_School/Ling_487/clean_code/Probe-HuBERT/TIMIT-Database/TIMIT/TEST/DR3/*/*.wav'))
...
lst = '/Users/kyleng/B_Organized/A_School/Ling_487/clean_code/Probe-HuBERT/TIMIT-Database/TIMIT/TEST/DR1/FJEM0/SX274.wav'
hs = len(Segs) * [[]]
for s in range(len(hs)):
    hs[s] = 25*[np.empty((1024, 0), float)]
    np.save("HS_"+Segs[s]+".npy", np.array(hs[s]))
for n in range(len(lst)):
    speech, rate = librosa.load(lst, sr=16000)
